 ...
 * @author victo
 */
public class VentanaEmpleadosConsultas extends javax.swing.JFrame {

    Empleados empleado;
    EmpleadoDAO eDAO = new EmpleadoDAO();
    int cont = 0;
    
    public VentanaEmpleadosConsultas() {
        initComponents();
    ...
    private void initComponents() {
        ...
        jPanel1.add(jLabel2);
        jLabel2.setBounds(240, 0, 300, 50);
    ...
    private void btnBuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBuscarActionPerformed
        ...
        if (!(tf_idempleado.getText().equals(""))) {
            consulta1 = consulta1 + "Id_Empleado = '" + tf_idempleado.getText() + "'";
            v = true;
        }
        if (!(tf_nombre.getText().equals(""))) {
            if(v){
                consulta1 = consulta1 + "AND Nombre = '" + tf_nombre.getText() + "'";
            }else{
                consulta1 = consulta1 + "Nombre = '" + tf_nombre.getText() + "'";
                v = true;
            }
        }
        if (!(tf_direccion.getText().equals(""))) {
            if(v){
                consulta1 = consulta1 + "AND Direccion = '" + tf_direccion.getText() + "'";
            }else{
                consulta1 = consulta1 + "Direccion = '" + tf_direccion.getText() + "'";
                v = true;
            }
        }
        if (!(tf_telefono.getText().equals(""))) {
            if(v){
                consulta1 = consulta1 + "AND Telefono = '" + tf_telefono.getText() + "'";
            }else{
                consulta1 = consulta1 + "Telefono = '" + tf_telefono.getText() + "'";
                v = true;
            }
        }
        if (!(tf_salario.getText().equals(""))) {
            if(v){
                consulta1 = consulta1 + "AND Salario = '" + tf_salario.getText() + "'";
            }else{
                consulta1 = consulta1 + "Salario = '" + tf_salario.getText() + "'";
                v = true;
            }
        }
        if (!(tf_ocupacion.getText().equals(""))) {
            if(v){
                consulta1 = consulta1 + "AND Ocupacion = '" + tf_ocupacion.getText() + "'";
            }else{
                consulta1 = consulta1 + "Ocupacion = '" + tf_ocupacion.getText() + "'";
            }
        }
        if(consulta1.equals("")){
            actualizarTablas(tabla1, consulta1);
        }else{
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new VentanaEmpleadosConsultas().setVisible(true);
            }
        });
    }
    ...
    public void actualizarTablas(JTable tabla, String consulta) {
        ...
        String consulta1 = "SELECT * FROM Empleados " + consulta;
        ResultSetTableModel modeloTabla = null;
        
        try {
            modeloTabla = new ResultSetTableModel(controlador, URL, consulta1);
    ...
    public boolean contarVacios(Component...componentes){
        cont = 0;
        for(Component x: componentes){
            if(x instanceof JTextField){
                if(((JTextField)x).getText().equals("")){
                    cont++;
                }
            }
        }//foreach
